...
var MidiIO = function () {
  ...
  MidiIO.getAllMetadata = function getAllMetadata(parsedMidi, tracks) {
    ...
    var indexedMeta = metaTrack.reduce(function (prev, current, i) {
      ...
      var updated = _Object$assign({}, prev, (_Object$assign2 = {}, _Object$assign2[current.type + '_' + current.subtype] = current, _Object$assign2));
      return updated;
    }, {});
    ...
    var instrumentNames = instrumentNumbers.map(function (instrumentNumber) {
      return _Object$keys(INSTRUMENT_MIDI_MAPPING).find(function (instrumentKey) {
  ...
  MidiIO.getAllTracks = function getAllTracks(parsedMidi) {
    ...
    var indexedTracks = tracks.map(function (track, i) {
      var indexedTrack = track.reduce(function (prev, current) {
        ...
        var updated = _Object$assign({}, prev);
      ...
      var instrumentName = _Object$keys(INSTRUMENT_MIDI_MAPPING).find(function (instrumentKey) {
        return INSTRUMENT_MIDI_MAPPING[instrumentKey] === instrumentNumber;
      });
      return {
  ...
  MidiIO.urlToBinaryString = function urlToBinaryString(midiURL) {
    return _Promise.resolve(true).then(function () {
      ...
      console.warn('ERROR IN URLTOBINARYSTRING, ' + _JSON$stringify(err));
    });
  };
  ...
  MidiIO.getTracksAndMetaFromParsedMidi = function getTracksAndMetaFromParsedMidi(parsedMidi) {
    var meta = parsedMidi.meta,
        musicTracks = parsedMidi.musicTracks;
    ...
    if (this.settings.midiURL) {
      return MidiIO.urlToJSON(this.settings.midiURL).then(function (parsedMidi) {
        return (
          ...
          _this.parseMidi(parsedMidi, _this.settings.BPM)
        );
      }).catch();
    }
