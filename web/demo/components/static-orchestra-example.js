...
const midiURL =
  "https://s3-eu-west-1.amazonaws.com/ut-music-player/assets/midis/Mr.+Robot+Main+Theme+midi.mid";
...
class StaticOrchestraExample extends React.Component {
  ...
  onMidiLoaded(parsedMidi) {
    console.warn(
      `Midi loaded ${JSON.stringify(
        parsedMidi,
        2,
        2
      )}. Loading instruments now ...`
    );
    return parsedMidi;
  }
  ...
  onNoteStopPlaying(instrumentName, noteName) {
    ...
    console.warn(
      `Note ${noteName} stopped playing, optionally handle this event`
    );
  }
  togglePlayback() {
  ...
  renderNote(instrumentName, noteName) {
    return (
      <div className="control">
        <div
          className={`button ${
            instrumentName + noteName in this.state.playingNotes
              ? "is-primary"
              : ""
          }`}
        >
